...
function TabsTrigger({
  ...
  return (
    <TabsPrimitive.Trigger
      ...
      className={cn(
        // Base styles
        'relative inline-flex h-[calc(100%-1px)] flex-1 items-center justify-center gap-1.5 rounded-md border border-transparent px-2 py-1 text-sm font-medium whitespace-nowrap transition-all cursor-pointer',
        // Text color: muted by default, full on hover
        'text-text-muted hover:text-text',
        // Vertical orientation
        'group-data-[orientation=vertical]/tabs:w-full group-data-[orientation=vertical]/tabs:justify-start',
        // Focus ring (ember)
        'focus-visible:border-ember focus-visible:ring-[3px] focus-visible:ring-ember/20 focus-visible:outline-1 focus-visible:outline-ember',
        // Disabled
        'disabled:pointer-events-none disabled:opacity-50',
        // Active state: default variant
        'group-data-[variant=default]/tabs-list:data-[state=active]:shadow-sm',
        'data-[state=active]:bg-surface-2 data-[state=active]:text-text',
        // Active state: line variant (no bg, no shadow)
        ...
        // Underline indicator (ember color)
        'after:absolute after:bg-ember after:opacity-0 after:transition-opacity',
